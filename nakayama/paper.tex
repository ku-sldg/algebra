...
\author{ 
    ...
    DMS-1945611, the Keeler Intra-University Professorship from the University of Kansas, and the 2022-23 Ruth I.\, Michler Memorial Prize from the
...
\subsection{Semigroups} Our foundation begins by defining a semigroup class, which declares a binary
...
%  proposed a rewrite of the blue subsubsection right above it.  Drew, can you
...
\section{Constructing the Formal Proof}
Not only to aid in the formalization of Nakayama's lemma, but also to 
...
\emily[inline]{Emily: Just changed statement to \emph{if and only if} below; we should check whether we should update its formal proof}
Suppose that $I$ is an ideal of a commutative rings $R$.  
We formally establish the following: If $I$ contains a
...
\begin{quote}
    ...
    which contradicts the assumption that $x$ is not a unit. Hence by the statement described earlier in this section, 
    $I_1$ is proper.
   
   Either $I_1$ is a maximal ideal of $R$, or it is strictly contained in a maximal ideal, or there exists 
   an inifinite chain of strictly increasing
   proper ideals of the form 
    \begin{equation} \label{infinite-chain: e}
    I_{1}\subsetneq I_{2}\subsetneq I_{3}\subsetneq\cdots\subsetneq I_n \subsetneq \cdots. 
    \end{equation}  
   Indeed, suppose that $I_1$ is not maximal, and   
   assume that for some integer $n \geq 1$,  there exist proper, non-maximal ideals 
   $I_k$ of $R$, $1 < k \leq n$, such that for all such $k$,  $I_{k-1}$ is strictly contained in $I_k$.  
...
%    
    ...
    is, by definition, an ideal containing every ideal in the union.  
Hence if, in this process, no maximal ideal is identified; i.e., we have ensured the existence of a chain of the form \eqref{infinite-chain: e},  then the increasing union of the $I_k$ is again an ideal, which contains $x$ since  $x \in I_1$. 
   
%  Continuing in this way, if  this process terminates, we have successfully identified a
%    maximal ideal of $R$ that contains $x$, and if not, one obtains an infinite, strictly increasing chain of proper ideals of $R$, each containing $x$, 
%    \[I_{1}\subsetneq I_{2}\subsetneq I_{3}\subsetneq\cdots\subsetneq I_n \subsetneq \cdots. \]
%    It is straightforward to verify that an increasing union of ideals 
...
%       contains \(x\) as well.  
      ...
      Hence the set $X$ of all proper ideals of $R$ containing $x$, partially ordered by inclusion, 
       satisfies the property that every chain in $X$ has an upper bound in $X$.
    Therefore, Zorn's lemma ensures the existence of a maximal element of $X$, i.e., a maximal ideal of \(R\) containing \(x\).
\end{quote}

The above argument has potentially infinitely many steps, posing a Coq implementation
 problem. Indeed, suppose one were to write a proof tactic called \mintinline{coq}{generate_larger_ideal} 
allowing one to move from a non-maximal, proper ideal \(I\) that contains a fixed non-unit 
...
After we have overwhelmed any system with
...
We now outline the formal proof of Nakayama's lemma, and  proceed
by induction on the number of elements in a fixed generating set for the finitely generated $R$-module
